use renclave_shared::{
    EnclaveOperation, EnclaveRequest, EnclaveResponse, EnclaveResult,
};
use renclave_enclave::seed_generator::SeedGenerator;
use renclave_enclave::quorum::{boot_genesis, GenesisSet, P256Pair, shares_reconstruct};
...
use renclave_shared::{QuorumMember, ManifestEnvelope, Manifest, NitroConfig, PivotConfig, ManifestSet, ShareSet};
...
async fn test_quorum_key_generation_integration() {
    ...
    // Note: Using None for storage to avoid permission issues in test environment
    let output = match boot_genesis(&genesis_set, None, None).await {
        Ok(output) => output,
...
async fn test_storage_integration() {
    ...
    // In test environment, files might exist from previous runs
    // Just verify that the storage state can be retrieved
    println!("Storage state: ephemeral_key_exists={}, quorum_key_exists={}", 
             state.ephemeral_key_exists, state.quorum_key_exists);
...
async fn test_complete_workflow_integration() {
    ...
    // 2. Validate seed
    let is_valid = seed_generator.validate_seed(&seed_result.phrase).await.unwrap();
    assert!(is_valid);
    
    // 3. Derive key from seed
    let key_result = seed_generator.derive_key(
        &seed_result.phrase,
        "m/44'/0'/0'/0/0",
        "secp256k1"
    ).await.unwrap();
...
async fn test_concurrent_operations_integration() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let passphrase = if i % 3 == 0 { Some(format!("pass-{}", i)) } else { None };
            
            let seed = generator.generate_seed(strength, passphrase.as_deref()).await.unwrap();
...
async fn test_error_handling_integration_advanced() {
    ...
    for (strength, passphrase) in error_cases {
        ...
        assert!(result.is_err(), "Should fail with invalid strength {}", strength);
    }
    ...
    for invalid_seed in invalid_seeds {
        ...
        assert!(!result, "Invalid seed should be invalid: '{}'", invalid_seed);
    }
}
...
async fn test_stress_test_integration() {
    ...
    for i in 0..1000 {
        ...
        let passphrase = if i % 5 == 0 { Some(format!("stress-{}", i)) } else { None };
        
        let seed = seed_generator.generate_seed(strength, passphrase.as_deref()).await.unwrap();
        let is_valid = seed_generator.validate_seed(&seed.phrase).await.unwrap();
        assert!(is_valid);
...
async fn create_test_seed_generator() -> SeedGenerator {
    SeedGenerator::new().await.expect("Failed to create test generator")
}
...
fn main() {
    ...
    println!("Integration tests enhanced - run with cargo test");
}
