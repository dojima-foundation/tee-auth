...
async fn test_complete_seed_generation_workflow() {
    ...
    let seed_result = seed_generator
        .generate_seed(256, Some("secure-passphrase-2024"))
        .await;
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let seed_result = generator
                .generate_seed(256, Some(&format!("passphrase-{}", i)))
                .await;
            assert!(seed_result.is_ok());
...
async fn test_seed_derivation_workflow() {
    ...
    let is_valid = seed_generator
        .validate_seed(&base_seed.phrase)
        .await
        .unwrap();
    assert!(is_valid);
...
fn main() {
    ...
    println!("E2E tests - run with cargo test");
}
