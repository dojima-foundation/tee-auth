...
output "next_steps" {
  ...
  value = <<EOF
GitHub Actions runner has been configured for development environment. Next steps:

1. Check your GitHub repository settings:
   - Go to: https://github.com/${var.github_repo}/settings/actions/runners
   - Verify that your development runner is online

2. Test your runner:
   - Push a commit to trigger a workflow
   - Check that jobs execute successfully on the dev runner

3. Monitor your runner:
   - Status page: ${join(", ", module.github_runners.runner_status_pages)}
   - SSH access: ssh -i runner_private_key.pem ubuntu@<runner-ip>

4. Use the helper scripts:
   - ./scripts/monitoring/check_runner_logs.sh <runner-ip> dev
   - make dev-status

Environment: Development
Runners: 1 instance (cost-optimized)
Labels: ${join(", ", var.runner_labels)}, dev
EOF
}
