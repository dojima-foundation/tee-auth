...
output "next_steps" {
  ...
  value = <<EOF
GitHub Actions runners have been configured for staging environment. Next steps:

1. Check your GitHub repository settings:
   - Go to: https://github.com/${var.github_repo}/settings/actions/runners
   - Verify that your staging runners are online

2. Test your runners:
   - Push a commit to trigger a workflow
   - Check that jobs execute successfully on staging runners
...
4. Use the helper scripts:
   - ./scripts/monitoring/check_runner_logs.sh <runner-ip> staging
   - make staging-status

Environment: Staging
Runners: ${var.runner_count} instances
Labels: ${join(", ", var.runner_labels)}, staging
EOF
}
